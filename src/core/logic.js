...
function getSychErrorReply(errText) {
    ...
    if (error.includes('prohibited') || error.includes('safety') || error.includes('blocked') || error.includes('policy')) {
        const phrases = [
            ...
            "🔞 Нет, ну так нельзя. Нейронка отказалась это генерить, говорит \"Violation of Safety Policy\". Слишком грубо.",
            "👮‍♂️ Гугл считает, что этот контент оскорбляет чувства. Попробуй помягче спросить."
        ];
    ...
    if (error.includes('503') || error.includes('overloaded') || error.includes('unavailable') || error.includes('timeout')) {
        const phrases = [
            "🔥 Сервера плавятся. Говорят \"Model is overloaded\". Подождите минуту, пусть остынут.",
            "🐌 Поисковик тормозит, 503-я ошибка.",
            "💤 Что-то нейронка устала. Пишет \"Service Unavailable\". Дайте ей отдохнуть немного."
        ];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    ...
    if (error.includes('429') || error.includes('quota') || error.includes('exhausted') || error.includes('лимит')) {
        const phrases = [
            "💸 У меня лимиты закончились, мы слишком много общались. Ждем отката квоты.",
            "🛑 Ошибка 429 — \"Too Many Requests\". Я слишком быстро отвечаю, меня притормозили. дайте отдышаться.",
            "📉 Квота по сообщениям закончилась. Попробуйте позже."
        ];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    ...
    if (error.includes('400') || error.includes('too large') || error.includes('invalid argument')) {
        const phrases = [
            ...
            "📜 Много букв, я так до утра читать буду. Ошибка \"Payload size limit\".",
            "💾 Файл слишком большой, в меня не влезает. Давай что-то полегче."
        ];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    ...
    const phrases = [
        "🛠  Админ, просыпайся, тут всё сломалось!",
        "💥 Я упал. Критическая ошибка. Я работать не могу.",
        "🚑 Хьюстон, у нас проблемы. Админ, выручай."
    ];
    return phrases[Math.floor(Math.random() * phrases.length)];
}
...
• «Советник напомни завтра в 10» — поставлю напоминание. Можно реплаем на сообщение с датой.

**🎲 Развлекуха:**
• "Советник кинь монетку" — Орёл/Решка.
• "Советник число 1-100" — Рандомное число в диапазоне.
• "Советник кто из нас [вопрос]" — Выберу случайного из чата.

**🕵️ Досье и Память:**
• "Советник кто я?" — Моё честное мнение о тебе.
• "Советник расскажи про @юзера" — Выдам досье на участника.
• "Советник стата" — Статистика токенов за сутки.
• "Советник, этот чат про [тема]" — Задать тему чата вручную.
...
_ver: ${config.version}_
  ...
  // === СТАТИСТИКА ===
  if (cleanText === 'Советник стата' || cleanText === 'Советник статистика') {
  ...
  // === ФИЧИ ===
  if (hasTriggerWord) {
      // Команда "Советник, этот чат про..." — используем оригинальный текст (не lowercase)
      ...
      const isWhoGame = cleanText.match(/(?:кто|кого)\s+(?:из нас|тут|здесь|в чате|сегодня)/) || cleanText.match(/Советник\W+кто\??$/) || cleanText.trim() === "Советник кто";
      if (isWhoGame) {
          try { await bot.sendChatAction(chatId, 'typing', getActionOptions(threadId)); } catch(e){}
  ...
  // === РЕШЕНИЕ ОБ ОТВЕТЕ ===
  // Бот отвечает ТОЛЬКО когда его явно вызвали (тег "Советник/Лисичка") или ответили на его сообщение
  ...
  if (shouldAnswer) {
    ...
    try {
        ...
        addToHistory(chatId, "Советник", aiResponse);
