...
• «Сова напомни завтра в 10» — поставлю напоминание. Можно реплаем на сообщение с датой.

**🎲 Развлекуха:**
• "Сова кинь монетку" — Орёл/Решка.
• "Сова число 1-100" — Рандомное число в диапазоне.
• "Сова кто из нас [вопрос]" — Выберу случайного из чата.

**🕵️ Досье и Память:**
• "Сова кто я?" — Моё честное мнение о тебе.
• "Сова расскажи про @юзера" — Выдам досье на участника.
• "Сова стата" — Статистика токенов за сутки.
• "Сова, этот чат про [тема]" — Задать тему чата вручную.
...
_ver: ${config.version}_
  ...
  // === СТАТИСТИКА ===
  if (cleanText === 'Сова стата' || cleanText === 'Сова статистика') {
  ...
  // === ФИЧИ ===
  if (hasTriggerWord) {
      // Команда "Сова, этот чат про..." — используем оригинальный текст (не lowercase)
      ...
      const isWhoGame = cleanText.match(/(?:кто|кого)\s+(?:из нас|тут|здесь|в чате|сегодня)/) || cleanText.match(/Сова\W+кто\??$/) || cleanText.trim() === "Сова кто";
      if (isWhoGame) {
          try { await bot.sendChatAction(chatId, 'typing', getActionOptions(threadId)); } catch(e){}
  ...
  // === РЕШЕНИЕ ОБ ОТВЕТЕ ===
  // Бот отвечает ТОЛЬКО когда его явно вызвали (тег "Сова/Манюня") или ответили на его сообщение
  ...
  if (shouldAnswer) {
    ...
    // 3. ВИДЕО
        ...
        if (vid.file_size > 20 * 1024 * 1024) {
            return bot.sendMessage(chatId, "🐢 Братан, видос жирный (больше 20мб). Я не грузчик, таскать такое. Сожми или обрежь.", getReplyOptions(msg));
        }
        try {
    ...
    try {
        ...
        addToHistory(chatId, "Сова", aiResponse);
